...
The terminal itself is ttyd's xterm.js, not this UI layer -- the wrapper bar, soft keys, compose bar and the landing page are what actually follow this document's components. But by default (`SERVERJACK_TERM_THEME` unset or not `off`, and no `theme=...` of your own already in `TTYD_EXTRA_ARGS` -- see below) `bin/serverjack` builds a color theme straight from the tokens above (`_term_theme()`, generated once -- the single place these colors are written down, not hand-typed a second time anywhere), so the terminal blends with the surrounding page instead of showing xterm.js's stock look next to it. `bin/serverjack-ttyd` asks for it as JSON (`python3 bin/serverjack --print-theme`) and passes it to ttyd as a real `-t theme=...` server option -- **not** a `?theme=...` URL query, which is how an earlier version of this delivered it: ttyd's client applies a URL query's theme last, which sounds like the more robust mechanism, but only works on ttyd clients new enough to read a URL query at all. Older ttyd -- still what some distros' package archives ship, and the exact mismatch that let this pass locally while failing CI -- silently ignores it, leaving the terminal in ttyd's stock look. A plain `-t theme=...` server option works on every ttyd version this project has ever supported.
...
Set `SERVERJACK_TERM_THEME=off` to skip generating this and get ttyd's stock xterm.js theme back (`--print-theme` prints nothing, so `bin/serverjack-ttyd` adds no `-t theme=...` at all). A `-t theme=...` (or `--client-option[=]theme=...`) of your own in `TTYD_EXTRA_ARGS` does the same automatically: `bin/serverjack` detects it (`_ttyd_extra_args_has_theme()`, reading the same `TTYD_EXTRA_ARGS` the wrapper does) and `--print-theme` prints nothing in that case either, so `bin/serverjack-ttyd` never prepends a second, conflicting `-t theme=...` ahead of yours -- ttyd keeps the *last* `-t theme=...` it sees, so an extra one from the generated theme would otherwise silently win over a hand-set one regardless of which started first (see the README's config table).

...
Set `SERVERJACK_TERM_THEME=off` to skip generating this and get ttyd's stock xterm.js theme back. A `-t theme=...` (or `--client-option[=]theme=...`) of your own in `TTYD_EXTRA_ARGS` does the same automatically: `bin/serverjack` detects it (`_ttyd_extra_args_has_theme()`) and skips appending its own `&theme=...` to the ttyd URL, so your theme -- not the generated one -- is what ttyd's client ends up applying. This matters because the URL query is what ttyd's client applies last: without that detection, the generated theme would silently win over a hand-set `-t theme=...` regardless of which one started first (see the README's config table).

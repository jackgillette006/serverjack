...
| `SERVERJACK_TERM_THEME` | unset (on) | `off` (or `0`/`no`/`false`) skips the default terminal color theme (built from the app's own tokens and passed to ttyd on the terminal iframe's URL), leaving ttyd's stock xterm.js look. The generated theme applies *after* anything `-t theme=...` in `TTYD_EXTRA_ARGS` sets, so it wins over a custom theme unless this is also `off`; combine both to use your own theme |
